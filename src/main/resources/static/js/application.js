...
function addSearchResult(i, data) {
    ...
    if ($.inArray(file_number, table_files) > -1) {
        ...
        $(".wxj_result").append("<tr class='temp'><td>" + item_id + "</td><td>" + description + "</td><td>" + revision + "</td><td>" + file_name + "</td><td>" + creator_id + "</td><td>"+release_status+"</td><td class='wxj_res_application' data-fileNumber=" + file_number + " data-number=" + number + ">添加</td></tr>");
    }
}
...
function addAddedList(i, data) {
    ...
    var release_status = data.releaseStatus;
    var number = i;
    $(".wxj_added_list").append("<tr ><td>" + item_id + "</td><td>" + description + "</td><td>" + revision + "</td><td>" + file_name + "</td><td>" + creator_id + "</td><td>"+release_status+"</td><td class='wxj_res_application delete' data-fileNumber=" + file_number + " data-number=" + number + ">删除</td></tr>");
...
function addFileList(data) {
    ...
    var item_id = data.itemId;
    var release_status = data.releaseStatus;
    $(".wxj_addBut_area").before("<div class='col-md-3 file_area_temp'><ul class='wxj_file_info_edit'><li><span>资料编号: </span><span>" + item_id + "</span></li><li><span>资料名称: </span><span>" + desc + "</span></li><li><span>资料版本: </span><span>" + revision + "</span></li><li><span>文件名: </span><span>" + file_name + "</span></li><li><span>创建者：</span><span>" + creator_id + "</span></li><li><span>状态：</span><span>" + release_status + "</span></li><p class='wxj_file_info_edit_del' data-fileNumber=" + file_number + ">删除</p></ul></div>");
}
...
function searchClick(page) {
    var fileType = $("#classification").text();
    var keyWord = $('#key_word').val();
    var isLastest = String($('#isLastest').prop('checked'));
